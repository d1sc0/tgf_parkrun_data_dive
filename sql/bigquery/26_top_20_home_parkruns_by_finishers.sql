-- Top 20 home parkruns by number of result rows.
...
WITH base AS (
  ...
  WHERE COALESCE(TRIM(home_run_name), '') <> ''
),
scored AS (
  SELECT
    *,
...
agg AS (
  SELECT
    ...
    ARRAY_AGG(
      STRUCT(first_name, last_name, club_name)
...
ranked AS (
  SELECT
    ROW_NUMBER() OVER (ORDER BY finishers_count DESC, home_run_name) AS rank_position,
    home_run_name,
    finishers_count,
    unique_athletes,
    FORMAT(
    ...
    latest_profile_sample.club_name AS latest_club_name
  FROM agg
)
SELECT *
FROM ranked
